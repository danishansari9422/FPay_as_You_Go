...
from users.models import donate
...
def update_ride_in_db_in(present):
	today=datetime.date.today()
	# cdd = datetime.datetime.now().day
	...
	for person in present:
		...
		try:
		   ...
		   ts = Time.objects.get(user=user, date = today)
		except:
			qs= None
			ts = None
		if qs is None:
			...
			if present[person]==True:
				...
				qs.total_due = 15
				qs.save(update_fields=['total_due'])
				ts.out=True
				ts.save(update_fields=['out'])
				# qs.pg_amt += qs.total_due
				qs.save(update_fields=['pg_amt'])
...
def fine(present):
	...
	for person in present:
		user = User.objects.get(username=person)
		try:
			qs = Present.objects.get(user=user)
			# , date = present[datetime.date]
		except:
			qs = None
		# if qs is None:
		# 	if present[person] == True:
		...
		if qs is not None:
			if present[person] == True:
				if today-qs.date >7:
					Begindate = datetime.strptime(qs.date, "%Y-%m-%d")
					enddate = Begindate + timedelta(days=7)
					fineamt=5*(today-timedelta(enddate))
				qs.total_due+=fineamt
				# qs.present = True
				qs.save(update_fields=['total_due'])
		# if present[person] == True:
...
# used
def hours_vs_date_given_passenger(present_qs, time_qs, admin=True):
	...
	plt.tight_layout()
	if (admin):
		plt.savefig('./recognition/static/recognition/img/ride_graphs/hours_vs_date/1.png')
		plt.close()
	else:
		plt.savefig('./recognition/static/recognition/img/ride_graphs/passenger_login/1.png')
		plt.close()
	return qs


# used
def hours_vs_passenger_given_date(present_qs, time_qs):
	register_matplotlib_converters()
	df_hours = []
	df_break_hours = []
	df_username = []
	...
	rcParams.update({'figure.autolayout': True})
	plt.tight_layout()
	plt.savefig('./recognition/static/recognition/img/ride_graphs/hours_vs_passenger/1.png')
	plt.close()
	return qs


def total_number_passengers():
	qs=User.objects.all()
	return (len(qs) -1)
...
def dashboard(request):
	if(request.user.username=='admin'):
		...
		return render(request,'recognition/passenger_dashboard.html')
...
def add_photos(request):
	...
	if request.method=='POST':
		...
		if username_present(username):
			...
			messages.warning(request, f'No such username found. Please register passenger first.')
...
def mark_your_ride(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Mark ride - In - Press q to exit",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @params with the millisecond of delay 1
		#cv2.waitKey(1)
		#To get out of the loop
...
@login_required
def view_ride_home(request):
	total_num_of_emp=total_number_passengers()
	emp_present_today=passengers_present_today()
	# this_week_emp_count_vs_date()
	# last_week_emp_count_vs_date()
	return render(request,"recognition/view_ride_home.html", {'total_num_of_emp' : total_num_of_emp, 'emp_present_today': emp_present_today})


@login_required
def view_ride_date(request):
	if request.user.username!='admin':
		return redirect('not-authorised')
	qs=None
	time_qs=None
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if(len(time_qs)>0 or len(present_qs)>0):
				qs=hours_vs_passenger_given_date(present_qs,time_qs)


				return render(request,'recognition/view_ride_date.html', {'form' : form,'qs' : qs })
			else:
				messages.warning(request, f'No records for selected date.')
				return redirect('view-ride-date')
...
@login_required
def view_ride_passenger(request):
	if request.user.username!='admin':
		return redirect('not-authorised')
	time_qs=None
	present_qs=None
		...
		if form.is_valid():
			...
			if username_present(username):
				...
				if date_to < date_from:
					...
					return redirect('view-ride-passenger')
				else:
					...
					if (len(time_qs)>0 or len(present_qs)>0):
						qs=hours_vs_date_given_passenger(present_qs,time_qs,admin=True)
						return render(request,'recognition/view_ride_passenger.html', {'form' : form, 'qs' :qs})
					else:
...
@login_required
def view_my_ride_passenger_login(request):
	if request.user.username=='admin':
		return redirect('not-authorised')
	qs=None
	time_qs=None
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if date_to < date_from:
					messages.warning(request, f'Invalid date selection.')
					return redirect('view-my-ride-passenger-login')
			else:
					...
					if (len(time_qs)>0 or len(present_qs)>0):
						qs=hours_vs_date_given_passenger(present_qs,time_qs,admin=False)
						return render(request,'recognition/view_my_ride_passenger_login.html', {'form' : form, 'qs' :qs})
					else:
...
def pg(request):
	if request.method == "POST":
		...
		info = Present(payment_name=name, total_due=amount)
		# , email = email
		#  , user_id = payment['id']
		info.save()

		return render(request, 'recognition/index.html', {'payment': payment})
